...
{
	//makin a lot of shit public HOPE IT DOEDSNT DO ANYTHING BAD LOL
	public static var instance:PlayState;

	public static var curStage:String = '';
	public static var SONG:SwaggiestSong;
	...
	public var playableCharacter:Character;
	...
	override public function create()
	{
		...
		weekData = Json.parse(Assets.getText(Paths.json('weeks/week' + storyWeek)));
		...
		playableCharacter = GameplayChangers.changers.get("Opponent Play") ? dad : boyfriend;
		...
		healthBarBG = new FlxSprite(0, (OptionsMenu.options.middleScroll ? FlxG.height / 2 : (OptionsMenu.options.downScroll ? 30 : FlxG.height * 0.95)) ).loadGraphic(Paths.image('healthBar'));
		...
		timeText = new FlxText(0, OptionsMenu.options.middleScroll ? OptionsMenu.options.downScroll ? 30 : FlxG.height * 0.95 : healthBarBG.y + (OptionsMenu.options.downScroll ? 15 : -30));
		timeText.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timeText.scrollFactor.set();

		scoreTxt = new FlxText(0, OptionsMenu.options.downScroll ? FlxG.height - 70 : 70, 0, "", 20);
		...
		if (OptionsMenu.options.songPosition)
			add(timeText);
		...
		//did it becaues its too long waklgedkl;jgsjlikvdc
		var author = weekData.songs[(isStoryMode ? curStorySong : FreeplayState.curSongSelected)].author;

		Lib.application.window.title = Application.current.meta.get("name") + " - " + SONG.song + " (" + storyDifficultyText +") | By: " + author;
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			hscript.call("countdown", [swagCounter]);
	...
	override public function update(elapsed:Float)
	{
		...
		if ((!GameplayChangers.changers.get("Opponent Play") && health > maxHealth) || (GameplayChangers.changers.get("Opponent Play") && health < maxHealth))
			health = maxHealth;
		...
		if (generatedMusic && FlxG.sound.music != null)
			timeText.text = FlxStringUtil.formatTime(Conductor.songPosition / 1000) + "/" + FlxStringUtil.formatTime(FlxG.sound.music.length/1000);
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.sections[Std.int(curStep / 16)].mustHit)
			{
			...
			if (PlayState.SONG.sections[Std.int(curStep / 16)].mustHit && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
		...
		if (camZooming)
		{
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom, FlxG.camera.zoom, CoolUtil.lerpShit(elapsed, 162.34));
			camHUD.zoom = FlxMath.lerp(1, camHUD.zoom, CoolUtil.lerpShit(elapsed, 162.34));
		}
		...
		{
			playableCharacter.stunned = true;
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				daNote.y = (daNote.mustPress ? playerStrums.members[daNote.noteData] : opponentStrums.members[daNote.noteData]).y + (OptionsMenu.options.downScroll ? 0.45 : -0.45)
				...
				if (OptionsMenu.options.downScroll)
				{
					if (daNote.isSustainNote) 
					{
						...
						if (GameplayChangers.changers.get("Bot Play") || !daNote.mustPress || daNote.wasGoodHit || (daNote.prevNote.wasGoodHit && !daNote.canBeHit) 
				...
				else if (GameplayChangers.changers.get("Bot Play") || daNote.isSustainNote && (!daNote.mustPress || daNote.wasGoodHit || 
		...
		hscript.call("updatePost", [elapsed]);
	}
	...
	{
		hscript.call("endSong", []);

		//DELETE THIS ITS GONNA GIVE YOU AN ERROR :SKULL:
		var data = {
			...
			footer: {
				text: "Song by: " + weekData.songs[(isStoryMode ? curStorySong : FreeplayState.curSongSelected)].author
			}
        };
		RequestShit.doReq(data);
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0)
			{
				...
				//week unlocked save data isnt even used i  dont want to use that shit, feel free to use it though idk
	...
	override function keyDown(event:KeyboardEvent)
	{
		...
		hscript.call("keyPress", [event]);

		//2d array
		var binds:Array<Array<FlxKey>> = [ //these should correspond to their note data i think??
		...
		//im too dumb to make my own note handler thing
		/*if (generatedMusic)
		{
			var notesToBeHit:Array<Note> = [];
			var notesThatCanBeHit:Array<Note> = [];

			notes.forEach(note -> {
				if (note.mustPress && note.canBeHit && !note.isSustainNote && !note.tooLate && !note.wasGoodHit && note.noteData == keyData)
					notesToBeHit.push(note);
			});

			notesToBeHit.sort((note,daNote) -> Std.int(note.strumTime - daNote.strumTime));

			if (notesToBeHit.length > 0)
			{
				boyfriend.holdTimer = 0;

				for (note in notesToBeHit)
				{
					notesThatCanBeHit.push(note);

					for (daNote in notesToBeHit) //possible stacked notes check
					{
						if (note == daNote) continue;

						//this will only run if there are more than 2 notes so yeah
						if (note.noteData == daNote.noteData)
							//note closest to strum will be hit first ??
							notesThatCanBeHit.push((Math.abs(Conductor.songPosition - note.strumTime) < Math.abs(Conductor.songPosition - daNote.strumTime) ? note : daNote));
					}
				}
			}
			else if (!OptionsMenu.options.ghostTapping)
			...
			if (notesThatCanBeHit.length > 0)
				for (coolNote in notesThatCanBeHit)
	...
	private function keyShit():Void
	{
		...
		if (playableCharacter.holdTimer > Conductor.stepCrochet * 0.004 && !pressed.contains(true) &&
			playableCharacter.animation.curAnim.name.startsWith('sing') && !playableCharacter.animation.curAnim.name.endsWith('miss'))
	...
	function noteMiss(direction:Int = 0):Void //might make this work with a note instead of direction
	{
		if (!playableCharacter.stunned)
		{
			hscript.call("noteMiss", [direction]);
			...
			{
				playableCharacter.stunned = false;
			});

			playableCharacter.playAnim('sing${animSuffix[direction]}miss', true);
		}
	}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.isChord)
			{
				if (!note.isSustainNote || playableCharacter.animation.curAnim.name.startsWith("sing" + animSuffix[note.noteData]))
					playableCharacter.holding = note.parentHold;
				playableCharacter.playAnim('sing' + animSuffix[note.noteData], true);
			}
	...
	{
		hscript.call("opponentNoteHit", [daNote]);
	...
	//everything from down here i might put in their respective scripts
	...
	override function beatHit()
	{
		...
		if (SONG.notes[Math.floor(curStep / 16)] != null)
		{
			...
			{
				Conductor.changeBPM(SONG.sections[Math.floor(curStep / 16)].changeBPM.bpm);
		...
		if (iconP1.scale.x < 1.24)
			iconP1.scale.set(1.24, 1.24);

		if (iconP2.scale.x < 1.24)
			iconP2.scale.set(1.24, 1.24);
