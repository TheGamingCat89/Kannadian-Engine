	...
	**/
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				case 'Must hit section':
					_song.sections[curSection].mustHit = check.checked;

					updateHeads();

				case 'Change BPM':
					_song.sections[curSection].changeBPM.active = check.checked;
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			{
				_song.sections[curSection].changeBPM.bpm = Std.int(nums.value);
				updateGrid();
			}
		}
	...
	function sectionStartTime():Float
	{
		...
		{
			if (_song.sections[i].changeBPM.active)
			{
				daBPM = _song.sections[i].changeBPM.bpm;
			}
	...
	override function update(elapsed:Float)
	{
		...
		strumLine.y = getYfromStrum((Conductor.songPosition - sectionStartTime()) % (Conductor.stepCrochet * _song.sections[curSection].lengthInSteps));
		...
		if (FlxG.mouse.wheel != 0 && !typingShit.hasFocus)
		{
			FlxG.sound.music.pause();
			vocals.pause();
	...
	function copySection(?sectionNum:Int = 1)
	{
		...
		for (note in _song.notes)
		{
			if (note.strumTime >= (Conductor.stepCrochet * _song.sections[curSection].lengthInSteps) * curSection  && note.strumTime <= (Conductor.stepCrochet * _song.sections[curSection].lengthInSteps) * (curSection + 1))
			{
				...
				var copiedNote:SwagNote = {strumTime: strum, noteData: note.noteData, sustainLength: note.sustainLength}
				_song.notes.push(copiedNote);
			}
		}
	...
	function updateSectionUI():Void
	{
		...
		check_mustHitSection.checked = sec.mustHit;
		check_altAnim.checked = sec.altAnim;
		check_changeBPM.checked = sec.changeBPM.active;
		stepperSectionBPM.value = sec.changeBPM.bpm;
	...
	function updateGrid():Void
	{
		...
		var sectionInfo:Array<SwagNote> = _song.notes;

		if (_song.sections[curSection].changeBPM.active && _song.sections[curSection].changeBPM.bpm > 0)
		{
			Conductor.changeBPM(_song.sections[curSection].changeBPM.bpm);
	...
	{
		for (_ in _song.notes)
			_song.notes = [];

		updateGrid();
	}

	private function addNote(time:Float, data:Int, susLen:Float = 0):Void
	{
		trace(data);
	...
	function getNotes():Array<SwagNote> return _song.notes;
...
// 		var clearSongButton:FlxButton = new FlxButton(reloadSongJson.x, reloadSongJson.y + 30, "Clear Song", clearSong);
...
// 				{
// 					FlxG.log.add('added note');
// 					addNote(getStrumTime(dummyArrow.y) + sectionStartTime(), Math.floor(FlxG.mouse.x / GRID_SIZE), Math.floor(FlxG.mouse.x / GRID_SIZE) < 3);
...
// 			var copiedNote = {noteData: note.noteData, sustainLength: note.sustainLength, strumTime: strum};
// 			_song.notes.push(copiedNote);
// 		}
...
// 		for (daNote in sectionInfo)
// 		{
// 			if (daNote.strumTime >= (Conductor.stepCrochet * _song.sections[curSection].lengthInSteps) * curSection  && daNote.strumTime <= (Conductor.stepCrochet * _song.sections[curSection].lengthInSteps) * curSection++)
// 			{
// 				var daNoteInfo = daNote.noteData;
// 				var daStrumTime = daNote.strumTime;
// 				var daSus = daNote.sustainLength;
...
// 			{
// 				curSelectedNote = _song.notes[swagNum];
// 			}
...
// 	function deleteNote(note:Note):Void
// 	{
// 		for (swagNote in _song.notes)
// 		{
// 			if (swagNote.strumTime == note.strumTime && swagNote.noteData == note.noteData)
// 			{
// 				FlxG.log.add('deleted note!!!');
// 				_song.notes.remove(swagNote);
// 			}
// 		}
...
// 	private function addNote(strumTime:Float, noteData:Int, mustHit:Bool, sus:Float = 0):Void
// 	{
// 		_song.notes.push({strumTime: strumTime, noteData: noteData, sustainLength: sus});

// 		curSelectedNote = _song.notes[_song.notes.length - 1];

// 		if (FlxG.keys.pressed.CONTROL)
// 		{
// 			_song.notes.push({strumTime: strumTime, noteData: (noteData + 4) % 8, sustainLength: sus});
// 		}
...
// 		addNote(Conductor.songPosition, keyData, true);
// 	}
...
// 	function getNotes():Array<SwagNote>
// 	{
// 		var noteData:Array<SwagNote> = [];

// 		for (i in _song.notes)
// 			if (i.strumTime >= (Conductor.stepCrochet * _song.sections[curSection].lengthInSteps) * curSection  && i.strumTime <= (Conductor.stepCrochet * _song.sections[curSection].lengthInSteps) * curSection++)
// 				noteData.push(i);
